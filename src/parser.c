...
static inline void on_number_end(p *parser) {
    ...
    on_token(parser, TLC_TOKEN_INTEGER, pos, len);
}
...
static inline void on_long_bracket_end(p *parser) {
    size_t pos = parser->word_start;
    int len = parser->parsed - pos + parser->long_br_level + 1; // ]===]
...
static void on_token_start(p *parser, uc c) {
    switch (c) {
        ...
        case '_':
            ...
            return;
        TLC_CASE_DIGIT:
            on_composite(parser, TLC_PARSER_STATE_NUMBER);
            return;
        case '\'':
...
static inline void on_long_bracket(p *parser, uc c) {
    if (c == ']' &&
        parser->parsed + parser->long_br_level < parser->file->size
    ) {
        on_long_bracket_end(parser);
        parser->parsed += parser->long_br_level + 1;
    }
}
...
static inline void on_long_bracket_start(p *parser, uc c) {
    switch (c) {
        case '=':
            return;
        case '[':
            parser->long_br_level = parser->parsed - parser->word_start - 2;
